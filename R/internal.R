...
#' Unzip a file
#'
#' Extract a file from a zip archive based on a file extension.
#'
#' @param x `character` zip file path.
#'
#' @param ext `character` file extension.
#'
#' @return `character` file path for unzipped file.
#'
#' @noRd
unzip_file <- funciton(x, ext) {
  ...
  # unzip archive
  unzip(self$attribute_path, exdir = td)
  # find file
  out <- dir(td, paste0("^.*\\.", ext), recursive = TRUE)
  # assert a file exists
  assertthat::assert_that(
    length(out) > 0,
    msg = "no file with specified extension in zip archive.")
  assertthat::assert_that(
    length(out) == 1,
    msg = "multiple files with specified extension in zip archive.")
  # return result
  out
}
