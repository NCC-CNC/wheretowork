...
The application is [available online](http://nature-vm01.carleton.ca/wheretowork). **Please note that you must use [Google Chrome](https://www.google.com/chrome/) for it to work.**.
...
This application can be deployed using a [nginx web server](https://www.nginx.com/). To achieve this, first install nginx with the following code:
...
Next, add the following text to the nginx configuration file (typically found at `/etc/nginx/nginx.conf`):
...
```
    ...
    server {
      ...
      location /wheretowork {
        ...
        proxy_set_header Host $host;3838
